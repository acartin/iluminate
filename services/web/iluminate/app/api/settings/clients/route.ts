...
export async function POST(request: Request) {
  ...
  if (placeholderAuthEnabled) {
    return redirectWithFeedback("info", `Placeholder mode: ${String(payload.name ?? "client")} was not saved to the database.`);
  }

  const response = await fetch(`${API_BASE_URL}/settings/clients`, {
    method: "POST",
    headers: {
  ...
  return redirectWithFeedback("success", "Client created successfully.");
}
