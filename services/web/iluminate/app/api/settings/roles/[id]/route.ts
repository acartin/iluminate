...
function redirectWithFeedback(type: "success" | "warning" | "error" | "info", message: string) {
  return redirectTo(`/settings/roles?${feedbackQuery(type, message)}`);
}
...
export async function POST(
  ...
  if (placeholderAuthEnabled) {
    return redirectWithFeedback("success", `Role ${id} updated in placeholder mode.`);
  }

  const response = await fetch(`${API_BASE_URL}/settings/roles/${encodeURIComponent(id)}`, {
    method: "PATCH",
    headers: {
  ...
  return redirectWithFeedback("success", "Role updated successfully.");
}
